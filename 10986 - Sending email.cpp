...
int dijkstra(int s, int t)
{
    ...
    while (!pq.empty()){
        ...
        pq.pop();
        if (dis[top]<top_cost) continue;
        int it;
        for (it=0; it<graph[top].size(); it++){
            int v=graph[top][it].first;
            int weight=graph[top][it].second;
            if (dis[top]+weight<dis[v]){
...
int main()
{
    int test, n, m, s, t, cnt=1;
    cin >> test;
    while(cnt<=test){
        for(int i=0; i<MM; i++) graph[i].clear();
        cin >> n >> m >> s >> t;
        while(m--){
            int a, b, w;
            cin >> a >> b >> w;
